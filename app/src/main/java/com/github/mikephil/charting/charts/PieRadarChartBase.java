 ...
 */
public abstract class PieRadarChartBase<T extends com.github.mikephil.charting.data.ChartData<? extends com.github.mikephil.charting.data.DataSet<? extends com.github.mikephil.charting.data.Entry>>>
        extends Chart<T> {
    ...
    public PieRadarChartBase(android.content.Context context, android.util.AttributeSet attrs) {
        super(context, attrs);
    }

    public PieRadarChartBase(android.content.Context context, android.util.AttributeSet attrs, int defStyle) {
    ...
    public void calculateOffsets() {
        ...
        if (mLegend != null && mLegend.isEnabled()) {
            ...
            if (mLegend.getPosition() == LegendPosition.RIGHT_OF_CHART_CENTER) {
                ...
                float spacing = com.github.mikephil.charting.utils.Utils.convertDpToPixel(13f);
                ...
                // this is the space between the legend and the chart
                float spacing = com.github.mikephil.charting.utils.Utils.convertDpToPixel(8f);

                float legendWidth = fullLegendWidth + spacing;

                float legendHeight = mLegend.mNeededHeight + mLegend.mTextHeightMax;

                android.graphics.PointF c = getCenter();

                android.graphics.PointF bottomRight = new android.graphics.PointF(getWidth() - legendWidth + 15, legendHeight + 15);
                float distLegend = distanceToCenter(bottomRight.x, bottomRight.y);

                android.graphics.PointF reference = getPosition(c, getRadius(),
                                                                getAngleForPoint(bottomRight.x, bottomRight.y));

                float distReference = distanceToCenter(reference.x, reference.y);
                float min = com.github.mikephil.charting.utils.Utils.convertDpToPixel(5f);
                ...
                android.graphics.PointF bottomLeft = new android.graphics.PointF(legendWidth - 15, legendHeight + 15);
    ...
    public float getAngleForPoint(float x, float y) {
        ...
        android.graphics.PointF c = getCenterOffsets();
     ...
     * @return
     */
    protected android.graphics.PointF getPosition(android.graphics.PointF center, float dist, float angle) {

        android.graphics.PointF p = new android.graphics.PointF((float) (center.x + dist * Math.cos(Math.toRadians(angle))),
    ...
    public void setRotationAngle(float angle) {
        ...
        mRotationAngle = com.github.mikephil.charting.utils.Utils.getNormalizedAngle(mRawRotationAngle);
    }
    ...
    public float getDiameter() {
        android.graphics.RectF content = mViewPortHandler.getContentRect();
     ...
     * @return
     */
    public java.util.List<com.github.mikephil.charting.utils.SelectionDetail> getSelectionDetailsAtIndex(int xIndex) {
        ...
        for (int i = 0; i < mData.getDataSetCount(); i++) {
            ...
            com.github.mikephil.charting.data.DataSet<?> dataSet = mData.getDataSetByIndex(i);
            ...
            vals.add(new com.github.mikephil.charting.utils.SelectionDetail(yVal, i, dataSet));
        }
    ...
    public void spin(int durationmillis, float fromangle, float toangle, Easing.EasingOption easing) {
        ...
        android.animation.ObjectAnimator spinAnimator = android.animation.ObjectAnimator.ofFloat(this, "rotationAngle", fromangle,
                                                                                                 toangle);
        spinAnimator.setDuration(durationmillis);
        spinAnimator.setInterpolator(Easing.getEasingFunctionFromOption(easing));

        spinAnimator.addUpdateListener(new android.animation.ValueAnimator.AnimatorUpdateListener() {
