...
public class LegendRenderer extends Renderer {
    ...
    public LegendRenderer(com.github.mikephil.charting.utils.ViewPortHandler viewPortHandler, Legend legend) {
        super(viewPortHandler);

        this.mLegend = legend;

        mLegendLabelPaint = new android.graphics.Paint(android.graphics.Paint.ANTI_ALIAS_FLAG);
        mLegendLabelPaint.setTextSize(com.github.mikephil.charting.utils.Utils.convertDpToPixel(9f));
        mLegendLabelPaint.setTextAlign(android.graphics.Paint.Align.LEFT);

        mLegendFormPaint = new android.graphics.Paint(android.graphics.Paint.ANTI_ALIAS_FLAG);
     ...
     * @param data
     */
    public void computeLegend(com.github.mikephil.charting.data.ChartData<?> data) {

        if (!mLegend.isLegendCustom()) {

            java.util.List<String> labels = new java.util.ArrayList<String>();
            java.util.List<Integer> colors = new java.util.ArrayList<Integer>();

            // loop for building up the colors and labels used in the legend
            for (int i = 0; i < data.getDataSetCount(); i++) {

                com.github.mikephil.charting.data.DataSet<? extends com.github.mikephil.charting.data.Entry> dataSet = data.getDataSetByIndex(i);

                java.util.List<Integer> clrs = dataSet.getColors();
                int entryCount = dataSet.getEntryCount();

                // if we have a barchart with stacked bars
                if (dataSet instanceof com.github.mikephil.charting.data.BarDataSet && ((com.github.mikephil.charting.data.BarDataSet) dataSet).isStacked()) {

                    com.github.mikephil.charting.data.BarDataSet bds = (com.github.mikephil.charting.data.BarDataSet) dataSet;
                    ...
                    if (bds.getLabel() != null) {
                        ...
                        colors.add(com.github.mikephil.charting.utils.ColorTemplate.COLOR_SKIP);
                        labels.add(bds.getLabel());
                    }

                } else if (dataSet instanceof com.github.mikephil.charting.data.PieDataSet) {

                    java.util.List<String> xVals = data.getXVals();
                    com.github.mikephil.charting.data.PieDataSet pds = (com.github.mikephil.charting.data.PieDataSet) dataSet;
            ...
            if (mLegend.getExtraColors() != null && mLegend.getExtraLabels() != null) {
                for (int color : mLegend.getExtraColors())
                    colors.add(color);
                java.util.Collections.addAll(labels, mLegend.getExtraLabels());
            }
        ...
        android.graphics.Typeface tf = mLegend.getTypeface();
    ...
    public void renderLegend(android.graphics.Canvas c) {
        ...
        float labelLineHeight = com.github.mikephil.charting.utils.Utils.getLineHeight(mLegendLabelPaint);
        float labelLineSpacing = com.github.mikephil.charting.utils.Utils.getLineSpacing(mLegendLabelPaint) + mLegend.getYEntrySpace();
        float formYOffset = labelLineHeight - com.github.mikephil.charting.utils.Utils.calcTextHeight(mLegendLabelPaint, "ABC") / 2.f;
        ...
        switch (legendPosition) {
            ...
            case ABOVE_CHART_CENTER: {
                ...
                com.github.mikephil.charting.utils.FSize[] calculatedLineSizes = mLegend.getCalculatedLineSizes();
                com.github.mikephil.charting.utils.FSize[] calculatedLabelSizes = mLegend.getCalculatedLabelSizes();
                ...
                for (int i = 0, count = labels.length; i < count; i++) {
                    ...
                    boolean drawingForm = colors[i] != com.github.mikephil.charting.utils.ColorTemplate.COLOR_SKIP;
            ...
            case LEFT_OF_CHART_INSIDE: {
                ...
                for (int i = 0; i < labels.length; i++) {
                    ...
                    if (labels[i] != null) {
                        ...
                        if (direction == Legend.LegendDirection.RIGHT_TO_LEFT)
                            x -= com.github.mikephil.charting.utils.Utils.calcTextWidth(mLegendLabelPaint, labels[i]);
     ...
     */
    protected void drawForm(android.graphics.Canvas c, float x, float y, int index, Legend legend) {

        if (legend.getColors()[index] == com.github.mikephil.charting.utils.ColorTemplate.COLOR_SKIP)
            return;
     ...
     */
    protected void drawLabel(android.graphics.Canvas c, float x, float y, String label) {
